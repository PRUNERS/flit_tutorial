...
We will go through three examples of using FLiT.
...
The files for this tutorial example can be found in `example_01/`.
...
look at how the FLiT test looks.  It is in `example_01/tests/Mfem13.cpp`
...
Looking first at `flit-config.toml`:
...
And that concludes the configurations we used for this tutorial example.  There
are other options that can be put into the file, which we deleted and fall back
to the default values.

Now, to look at `custom.mk`:
...
That is the entire `custom.mk` file for this tutorial example.  It begins by
...
Let's now change into the `example_01` directory:
...
The autogenerated Makefile may not be in your `example_01` directory.  This is
...
You will probably want more processes than 1, but for this tutorial example, we
will stick to this in order to be nice to others using the same machine.
...
For this example, we will go into a different directory:
...
```bash
$ diff -u ../example_01/custom.mk ./custom.mk
--- ../example_01/custom.mk	2019-07-01 16:09:39.239923037 -0600
...
Searching for differing source files:
  ...
  Created ./bisect-01/bisect-make-04.mk - compiling and running - score 193.00735125466363
    Found differing source file /home/bentley/git/pearc_flit_examples/packages/mfem/linalg/densemat.cpp: score 193.00735125466363
...
all variability inducing source file(s):
  /home/bentley/git/pearc_flit_examples/packages/mfem/linalg/densemat.cpp (score 193.00735125466363)
Searching for differing symbols in: /home/bentley/git/pearc_flit_examples/packages/mfem/linalg/densemat.cpp
  ...
  All differing symbols in /home/bentley/git/pearc_flit_examples/packages/mfem/linalg/densemat.cpp:
...
All variability inducing symbols:
  /home/bentley/git/pearc_flit_examples/packages/mfem/linalg/densemat.cpp:3692 _ZN4mfem13AddMult_a_AAtEdRKNS_11DenseMatrixERS0_ -- mfem::AddMult_a_AAt(double, mfem::DenseMatrix const&, mfem::DenseMatrix&) (score 193.00735125466363)
```
...
That concludes our tutorial examples.  Please visit the
